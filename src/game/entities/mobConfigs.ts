...
import { buildMobMeshFrom } from './mobModelMesh';

export const MOB_CONFIGS: Record<MobKind, MobConfig> = {
  cow: {
    kind: 'cow', behavior: 'passive', maxHealth: 10, speed: 1.6,
    size: { halfWidth: 0.45, height: 1.4 }, buildMesh: buildMobMeshFrom('cow', 1.4, buildCowMesh),
  ...
  pig: {
    ...
    size: { halfWidth: 0.4, height: 1.0 }, buildMesh: buildMobMeshFrom('pig', 1.0, buildPigMesh),
    attackDamage: 0, attackRange: 0, sightRange: 0,
    drops: [{ itemId: 'raw_porkchop', min: 1, max: 3 }],
  },
  sheep: {
    kind: 'sheep', behavior: 'passive', maxHealth: 8, speed: 1.5,
    size: { halfWidth: 0.4, height: 1.1 }, buildMesh: buildMobMeshFrom('sheep', 1.1, buildSheepMesh),
    attackDamage: 0, attackRange: 0, sightRange: 0,
    drops: [{ itemId: 'wool', min: 1, max: 2 }],
  },
  chicken: {
    kind: 'chicken', behavior: 'passive', maxHealth: 4, speed: 1.4,
    size: { halfWidth: 0.25, height: 0.7 }, buildMesh: buildMobMeshFrom('chicken', 0.7, buildChickenMesh),
    attackDamage: 0, attackRange: 0, sightRange: 0,
    drops: [{ itemId: 'raw_chicken', min: 1, max: 1 }, { itemId: 'feather', min: 0, max: 2 }],
  },
  zombie: {
    kind: 'zombie', behavior: 'hostile', maxHealth: 20, speed: 1.9,
    size: { halfWidth: 0.3, height: 1.8 }, buildMesh: buildMobMeshFrom('zombie', 1.8, buildZombieMesh),
  ...
  skeleton: {
    ...
    size: { halfWidth: 0.3, height: 1.8 }, buildMesh: buildMobMeshFrom('skeleton', 1.8, buildSkeletonMesh),
  ...
  spider: {
    ...
    size: { halfWidth: 0.5, height: 0.7 }, buildMesh: buildMobMeshFrom('spider', 0.7, buildSpiderMesh),
  ...
  creeper: {
    ...
    size: { halfWidth: 0.35, height: 1.7 }, buildMesh: buildMobMeshFrom('creeper', 1.7, buildCreeperMesh),
